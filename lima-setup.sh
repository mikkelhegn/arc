...
#az appservice plan create -g $groupName -n "${prefix}-app-plan" -l ${k8seLocation} --kube-environment "${kubeEnvironmentName}" --kube-sku ANY
#az webapp create -g $groupName -p "${prefix}-app-plan" -n "${prefix}-nginx" -l ${k8seLocation} --deployment-container-image-name docker.io/nginx:latest
